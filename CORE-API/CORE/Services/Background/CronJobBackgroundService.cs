    ...
    public class CronJobBackgroundService : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                if (now > _nextRun)
                {
                    _logger.LogInternalMessage("Running Cron");
                    _nextRun = _schedule.GetNextOccurrence(DateTime.Now);
                }
                _logger.LogInternalMessage("Sleeping for: " + (_nextRun - now));
                await Task.Delay((_nextRun - now), stoppingToken); //sleep until next occurance
